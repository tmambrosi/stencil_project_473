 ...
 * 
 *   Modified by Grace Mclewee and Tanner Ambrosino
 *
 *   Last author modification: 4 September 2002
 */
   ...
   int          cols)    /* OUT - Matrix cols */
{
   ...
   //printf("write_submatrix mympi.c\n");

   for (i = 0; i < rows; i++) {
      for (j = 0; j < cols; j++) {
     // W   printf("%d", a[i][j]);
         fwrite(&(a[i][j]), sizeof(double), 1, s);
         // wrote = wrote + w;
         //printf("%f ", a[i][j]);
      }//printf("\n");
   }
   //printf("wrote %d things mpi\n", wrote);
}


void write_row_striped_matrix (
   char *s,             /* IN – output filename */	
   double **a,            /* IN - 2D array */
   MPI_Datatype dtype,  /* IN - Matrix element type */
   int m,               /* IN - Matrix rows */
   int n,               /* IN - Matrix cols */
   MPI_Comm comm)       /* IN - Communicator */
{
   ...
   if (!id) {
      ...
      //printf("first call to write\n");
      //print_submatrix (a, dtype, local_rows, n);
      write_submatrix (f, a, dtype, local_rows, n);
      //printf("wrote data to file 1\n");
      //write matrix
      if (p > 1) {
         datum_size = get_size (dtype);
         max_block_size = BLOCK_SIZE(p-1,p,m);
         ...
         for (i = 1; i < p; i++) {
            ...
            //printf("i = %d \n", i);
            //printf("second call to write\n");
            write_submatrix (f, b, dtype, BLOCK_SIZE(i,p,m), n);
...
void write_row_striped_matrix_data (
   char *s,             /* IN – output filename */	
   double **a,            /* IN - 2D array */
   MPI_Datatype dtype,  /* IN - Matrix element type */
   int m,               /* IN - Matrix rows */
