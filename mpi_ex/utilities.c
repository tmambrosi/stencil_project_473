...
int stencil_cust_mpi(matrix_t *x, matrix_t *newx, int firstrow, int lastrow, double* upper_tmp, double*lower_tmp){
    ...
    for(int j = 1 ; j < (x->col-1) ; j++){
        //printf("accessing element: %d, %d\n", i, j);
        (newx->m)[i][j] =  ((upper_tmp)[j-1] + upper_tmp[j] + (upper_tmp)[j+1] + \
                            (old)[i][j+1] + (old)[i+1][j+1] + (old)[i][j] + \
                            (old)[i+1][j-1] + (old)[i+1][j] + (old)[i+1][j+1])/9.0;
    }
    ...
    for(int j = 1 ; j < (x->col-1) ; j++){
        ...
        (newx->m)[i][j] =  ((old)[i-1][j-1] + (old)[i-1][j] + (old)[i-1][j+1] + \
                            (old)[i][j+1] + (old)[i][j+1] + (old)[i][j] + \
                            (lower_tmp)[j-1] + (lower_tmp)[j] + (lower_tmp)[j+1])/9.0;
    }
...
void *pthstencil(void* my_data){
    ...
    long my_rank = (long) thread_d->rank;
    ...
    for(int i = 0; i < share_data.num_iter; i++){
        ...
        if(my_rank == 0){
            ...
            if(share_data.file != NULL){
                write2D(share_data.file, *(share_data.matrix_a));
            }
            //printf("swapped \n");
        }
...
/*
int mpi_stencil(int id, int p, matrix_t *a, matrix_t *b){

    if(a == NULL || a->m == NULL || a->row == NULL){
        return 1;
    }
    if(b == NULL || b->m == NULL|| b->row == NULL){
        return 1;
    }
    ...
    //send data tag = 0
    if(id == 0){
        int MPI_Ssend(&lower_tmp_send, 1, MPI_DOUBLE, id+1, 0, MPI_COMM_WORLD);
    }

    if(id == p-1){
        int MPI_Ssend(&upper_tmp_send, 1, MPI_DOUBLE, id-1, 0, MPI_COMM_WORLD);
    ...
    //recieve data
    if(id == 0){
        MPI_Recv(&lower_tmp_store, 1, MPI_DOUBLE, id+1, 0, MPI_COMM_WORLD, MPI_Status* status);
    }
    if(id == p-1){
        MPI_Recv(&upper_tmp_store, 1, MPI_DOUBLE, id-1, 0, MPI_COMM_WORLD, MPI_Status* status);
    }
    else{
        MPI_Recv(&lower_tmp_store, 1, MPI_DOUBLE, id+1, 0, MPI_COMM_WORLD, MPI_Status* status);
        MPI_Recv(&upper_tmp_store, 1, MPI_DOUBLE, id-1, 0, MPI_COMM_WORLD, MPI_Status* status);
    ...
    //do stencil iteration
    stencil_cust_mpi(a, b, my_first_row, my_last_row, upper_tmp_store, lower_tmp_store);

    //swap

    // if my id = 0:
        // set up buffer to gather data
        // we'll put it straight into the b matrix
    MPI_Gatherv(b->m[my_first_row][0], int sendcount, MPI_Datatype sendtype,
    void *recvbuf, const int recvcounts[], const int displs[], MPI_Datatype
recvtype,
    int root, MPI_Comm comm)

    //node 0 will broadcast matrix B as their matrix A
